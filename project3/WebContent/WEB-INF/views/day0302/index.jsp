...
<body>
	...
	<div class="blockquote main">
	   ...
	   <form id="frm" action="login.do" method="post">
	     <table>
	       <tr>
	          <td><input type="text" name="id" class="inputBox" id="id" style="height:30px; margin-top: -30px"/></td>
	          <td rowspan="2">
	          <input type="button" value="로그인" class="btn" id="btnLogin" style="height:66px;  width:60px; margin-top: 5px"/>
	          </td>
	       </tr>
	       <tr>
	          <td><input type="password" name="passwd" id="passwd" class="inputBox" style="height:30px; margin:0px"/></td>
	       </tr>
	     </table>
	     </form>
	     </c:if>
	     <c:if test="${not empty sessionScope.user_name }">
	        ${sessionScope.user_id}(${sessionScope.user_name })님 로그인 하셨습니다.<br/>
	     </c:if>
	     회원가입 | 아이디찾기 | 비밀번호찾기
	   </div>
	</div>
